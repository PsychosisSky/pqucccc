...
/* 1.14.5: set the wallet's discard threshold to 1 PQU because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 PQU. Very network
...
/*
 * Pqucoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Pqucoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Pqucoin: Get a fee targetting a specific transaction speed.
     */
    CAmount GetPqucoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Pqucoin: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetPqucoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Pqucoin dust fee.
     */
