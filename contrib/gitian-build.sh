...
# Dependencies
ossPatchUrl="https://depends.pqucoincore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.pqucoincore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.pqucoincore.org/MacOSX10.11.sdk.tar.gz"
...
SIGNER=
VERSION=
url=https://github.com/pqucoin/pqucoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of Pqucoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/pqucoin/pqucoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/pqucoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/pqucoin/gitian.sigs.git
    git clone https://github.com/pqucoin/pqucoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/pqucoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean pqucoin git directory because of old caching
    if [ -d inputs/pqucoin/ ]; then
        echo "Cleaning Pqucoin directory..."
        rm -rf inputs/pqucoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit pqucoin="$COMMIT" --url pqucoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
