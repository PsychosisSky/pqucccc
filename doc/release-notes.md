Pqucoin Core version 1.14.6 is now available from:

  <https://github.com/pqucoin/pqucoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Pqucoin Core users - miners, services,
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/pqucoin/lists/pqucoin-releases>
...
Pqucoin network from 1 PQU to 0.01 PQU. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 PQU as
   change if possible.
2. If after signing there is more than 0.01 PQU left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 PQU, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 PQU will not be accepted as valid by default.
...
```
pqucoin-cli addnode <address> remove
```
...
```
pqucoin-cli liststucktransactions
```
...
```
pqucoin-cli setmaxconnections <desired_amount>
```
...
```
pqucoin-cli rescan <start_block>
```
...
efforts to make Pqucoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Pqucoin Core.
