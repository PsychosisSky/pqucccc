...
Pqucoin Core 1.8.1 is primarily a bugfix release, bringing Pqucoin Core in
line with Bitcoin 0.9.3. Pqucoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Pqucoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Pqucoin Core clients
* Use HTTPS with pquchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/pqucoin/pqucoin/issues/134).
...
This only affects values significantly below 1 Pqu, however merchants may wish to round to
the nearest Pqu on all transactions for now.
