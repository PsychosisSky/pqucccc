...
using namespace std;
unordered_map<string, pair<uint32_t, uint32_t>> hash_too_nBits;
/**
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (!nMineAuxPow) {
            while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, Params().GetConsensus(nHeight))) {
                ++pblock->nNonce;
                --nMaxTries;
                std::cout << "nNonce: " << pblock->nNonce << std::endl;
            }
        } else {
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated pqucoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    //     throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Pqucoin is not connected!");

    // if (IsInitialBlockDownload())
    //     throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Pqucoin is downloading blocks...");
...
void AuxMiningCheck()
{
    ...
    //                        "Pqucoin is downloading blocks...");sss
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey)
{
     ...
     * a single pqucoind instance, for example when a pool runs multiple sub-
...
//cmx_newnewAuxMiningCreatBlock_define
static UniValue newAuxMiningCreateBlock(const CScript& scriptPubKey, uint256 &hash, uint32_t &nbits, int id)
{
    // AuxMiningCheck();
    // LOCK(cs_auxblockCache);
    static unsigned nTransactionsUpdatedLast;
    ...
    // Pqucoin: Never mine witness txs
    const bool fMineWitnessTx = false;
    ...
    if (iter != curBlocks.end()) pblock = iter->second;
    {
                ...
                && GetTime() - nStart > 60))
        {
            ...
            // Create new block with nonce = 0 and extraNonce = 1
            std::unique_ptr<CBlockTemplate> newBlock
                = BlockAssembler(Params()).CreateNewBlock(scriptPubKey, fMineWitnessTx, id, nbits);
    ...
    nbits = pblock->nBits;
    if (id == 1) {
            hash_too_nBits[hash.GetHex()].first = nbits;
        } else {
            hash_too_nBits[hash.GetHex()].second = nbits;
        }
...
int SubmitAuxBlock(const char buf[])
{
    ...
    //child hash
    std::cout << "\n\n\n====================================================================" << std::endl;
    std::cout << "child hash： " << hash.GetHex() << std::endl;
    ...
    ptr += 4;
    std::cout << "bitcoin nVersion: " << block.auxpow->parentBlock.nVersion << std::endl;
    std::cout << "size: " << sizeof(block.auxpow->parentBlock.nVersion)  << std::endl;
    int ret = 0;
    if (block.auxpow->parentBlock.nVersion == 0x10000000) {
        ret = 1;
    } else if (block.auxpow->parentBlock.nVersion == 0x20000000 ) ret = 2;   
    ...
    ptr += 32;
    std::cout << "parentBlock.hashPrevBlock: " << block.auxpow->parentBlock.hashPrevBlock.GetHex() << std::endl;
    std::cout << "size: " << block.auxpow->parentBlock.hashPrevBlock.size()  << std::endl;

    //parent hashMerkleRoot
    memcpy(block.auxpow->parentBlock.hashMerkleRoot.begin(), ptr, 32);
    ptr += 32;
    std::cout << "parentBlock.hashMerkleRoot: " << block.auxpow->parentBlock.hashMerkleRoot.GetHex() << std::endl;
    std::cout << "size: " << block.auxpow->parentBlock.hashMerkleRoot.size()  << std::endl;
   
    
    //nTime
    memcpy(&block.auxpow->parentBlock.nTime, ptr, 4);
    ptr += 4;
    std::cout << "nTime: " << block.auxpow->parentBlock.nTime << std::endl;
    std::cout << "size: " << sizeof(block.auxpow->parentBlock.nTime)  << std::endl;

    //nBits
    memcpy(&block.auxpow->parentBlock.nBits, ptr, 4);
    ptr += 4;
    std::cout << "nBits: " << block.auxpow->parentBlock.nBits << std::endl;
    std::cout << "size: " << sizeof(block.auxpow->parentBlock.nBits)  << std::endl;

    //nNonce
    memcpy(&block.auxpow->parentBlock.nNonce, ptr, 4);
    ptr += 4;
    std::cout << "nNonce: " << block.auxpow->parentBlock.nNonce << std::endl;
    std::cout << "size: " << sizeof(block.auxpow->parentBlock.nNonce)  << std::endl;

    // parent hash 没啥用只是为了打印，后边会用 coinbase hash覆盖
    memcpy(block.auxpow->hashBlock.begin(), ptr, 32);
    ptr += 32;
    std::cout << "parent hash: " << block.auxpow->parentBlock.GetHash().GetHex() << std::endl;
    std::cout << "size: " << block.auxpow->parentBlock.GetHash().size()  << std::endl;
    ...
    //std::cout << "v size:" << block.auxpow->vMerkleBranch.size() << std::endl;
    ...
    memcpy(block.auxpow->hashBlock.begin(), ptr, 32);
    ptr += 32;
    std::cout << "coinbase hash: " << block.auxpow->hashBlock.GetHex() << std::endl;
    uint32_t coinBaseSize;
    memcpy(&coinBaseSize, ptr, 4);
    ptr += 4;
    std::cout << "coinbase size: " << coinBaseSize << std::endl;
    std::cout << "====================================================================\n\n\n" << std::endl;
    // string str;
    ...
    UnregisterValidationInterface(&sc);
    if (fAccepted) {
        cout << "ProcessNewBlock sucess: " << ++count << endl;
        return ret;
    } else return 0;
}
...
UniValue createauxblock(const JSONRPCRequest& request)
{
      if (request.fHelp || request.params.size() != 4)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. address      (string, required) specify coinbase transaction payout address\n"
            "2.hash           (string, required) specify the hash of the block to be created\n"
            "3.nbits          (numeric, required) specify the nbits of the block to be created\n"
            "4.id             (numeric, required) specify the id of the block to be created\n"
            "\nResult:\n"
            "{\n"
            "  \"hash\"               (string) hash of the created block\n"
            "  \"chainid\"            (numeric) chain ID for this block\n"
    ...
    string strnbits = request.params[2].get_str();
    
    string strid = request.params[3].get_str();
    
    if (!coinbaseAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_PARAMETER,"Invalid coinbase payout address");
    uint32_t nbits = stringToUint32(strid);
    int id =stringToInt(strid);
    const CScript scriptPubKey = GetScriptForDestination(coinbaseAddress.Get());
    
    return newAuxMiningCreateBlock(scriptPubKey,hash,nbits,id);
}
