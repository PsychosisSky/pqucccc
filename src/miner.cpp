...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    coinbaseTx.vout[0].nValue = nFees + GetPqucoinBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash(
));
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, int id, uint32_t &nbits)
{
    int64_t nTimeStart = GetTimeMicros();
    ...
    //子链上第一个来自父链的块，初始难度设置为子链当前难度
    if (ltc_nBits.empty()) ltc_nBits.push_back(pblock->nBits);
    if (btc_nBits.empty()) btc_nBits.push_back(pblock->nBits);

    if (id == 1) {
        //count数组存储每分钟出的块的数量，至少需要四分钟的数据
        if (ltc_block_counts.size() >= 4 && pindexPrev->nHeight % 4 == 0) {
            CalculatecoinNextWorkRequired_only_aux(pindexPrev, consensus);
        } 
        nbits = ltc_nBits.back();
    } else if (id == 2) {
        if (btc_block_counts.size() >= 4 && pindexPrev->nHeight % 4 == 0) {
            CalculatecoinNextWorkRequired_only_aux(pindexPrev, consensus);
