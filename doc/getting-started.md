...
This tutorial will help you to go through the basics to use Pqucoin Core after you completed the [installation instructions](/INSTALL.md). You now have `pqucoind` or `pqucoin-qt` executables available to run a node, and `pqucoin-cli`/`pqucoin-tx` tools to help you transact PQU.
...
> ```console
> shibetoshi:~$ /path/to/pqucoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a pqucoin node](#starting-a-pqucoin-node)
...
## Starting a Pqucoin node

To start your node, you can run an headless server using `pqucoind`:
```console
shibetoshi:~$ pqucoind -daemon
```

Or you can use the Graphical User Interface (GUI), `pqucoin-qt`:
```console
shibetoshi:~$ pqucoin-qt
```
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Pqucoin network!

> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `pqucoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `pqucoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Pqucoin Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Pqucoin Core installation provides the `pqucoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ pqucoin-cli help

#Get help for a specific command
shibetoshi:~$ pqucoin-cli help COMMAND
```
...
To receive PQU, you need an address that is securely derived from a private key through a series of automatic, cryptographic operations. The *address* can be shared with anyone to receive PQU, but the *private key* is  sensitive information that allows anyone that knows it to spend the PQU on the associated address.

By default, the Pqucoin Core software will automatically create an address for you and securely store the private key in the wallet file.

You can list wallet addresses using `getaddressesbyaccount`:

```console
shibetoshi:~$ pqucoin-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ pqucoin-cli getnewaddress
...
```console
shibetoshi:~$ pqucoin-cli backupwallet /path/of/wallet/backup
```

**Tip:** Pqucoin addresses start with the letter `D`.

You now have two wallet addresses to share with other people to receive PQU! Consider avoiding [address reuse](https://en.bitcoin.it/wiki/Address_reuse) for anonymity and security reasons.
...
```console
#Syntax
shibetoshi:~$ pqucoin-cli getbalance "*" minconf
```
...
```console
shibetoshi:~$ pqucoin-cli getbalance "*" 5
421.552000
```

### Sending transactions

Pqucoin implements the [Unspent Transaction Output (UTXO)](https://en.wikipedia.org/wiki/Unspent_transaction_output) model to track which amounts of coin belong to an address. Owning PQU means that you know the private key(s) to addresses that are associated with unspent outputs. To spend them, you have to compose a new transaction that spends the value from currently unspent outputs to new outputs.
...
```console
#Syntax
shibetoshi:~$ pqucoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ pqucoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ pqucoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ pqucoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ pqucoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ pqucoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `pqucoin-cli decoderawtransaction` or `pqucoin-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Pqucoin Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
shibetoshi:~$ pqucoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ pqucoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ pqucoin-cli sendrawtransaction signed_transaction
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Pqucoin Core software to re-index the entire blockchain, and can take up to several hours.

First, request the information about block 69:

```console
#Find block hash from his height
shibetoshi:~$ pqucoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ pqucoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ pqucoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ pqucoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `pqucoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ pqucoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `pqucoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.pqucoin` by default, or specify the file location with `-conf`.

**pqucoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/pqucoin.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Pqucoin chain, it is recommended to not use the main Pqucoin network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `pqucoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `pqucoin-cli`.

### Data directory

The data directory is the location where Pqucoin Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.
...
Running Pqucoin Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Pqucoin Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
