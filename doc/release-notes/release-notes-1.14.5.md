Pqucoin Core version 1.14.5 is now available from:

  <https://github.com/pqucoin/pqucoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Pqucoin Core users, miners, services, relay
operators and wallet users are strongly recommended to upgrade.

Please report bugs using the issue tracker at github:

  <https://github.com/pqucoin/pqucoin/issues>

To receive security and update notifications, please watch reddit or Twitter:

  * https://www.reddit.com/r/pqucoindev/
  * @Pqucoin on Twitter for high priority announcements
  * @pqucoin\_devs on Twitter for updates on development work

Compatibility
==============

Pqucoin Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.

Pqucoin Core should also work on most other Unix-like systems but is not
...
most Pqucoin Core users.

### Remote Code Execution in Pqucoin QT (CVE-2021-3401)
...
**Pqucoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Pqucoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Pqucoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Pqucoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 PQU/kb, and
* the recommended dust limit is 1 PQU, and
* the recommended RBF increment is 0.001 PQU.

### Wallet/UI Changes

* The user interface for selecting fees when transacting PQU has been updated
  to give an idea of how much is being spent, rather than a block target. As
  Pqucoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Pqucoin.
...
* The default minimum transaction fee is now 0.01 PQU per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 PQU, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 PQU.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 PQU and sets the value under
...
The Berkley DB version used by Pqucoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within Pqucoin Core.
