...
Below are some notes on how to build Pqucoin Core for Windows.
...
Note that for WSL v1 the Pqucoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/pqucoin, AND not under, for example, /mnt/d/pqucoin. 
...
way. This will install to `c:\workspace\pqucoin`, for example:

    make install DESTDIR=/mnt/c/workspace/pqucoin
